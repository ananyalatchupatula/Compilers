...
void Ternary_Expr_Ast::pre_allocate_temps() {
    // IMPORTANT: Allocate this ternary's result stemp FIRST in pre-order,
    // so outer ternaries get lower stemp ids than any nested ternaries.
    if (result_stemp_id == -1) {
        TAC_Opd* result_stemp = TAC_Generator::get_instance()->create_new_stemp(
            node_data_type == FLOAT_DATA_TYPE
        );
        Temp_TAC_Opd* temp = dynamic_cast<Temp_TAC_Opd*>(result_stemp);
        if (temp) {
            result_stemp_id = temp->get_temp_id();
        }
        delete result_stemp;
    }

    // Pre-allocate condition next (may contain nested ternaries)
...
TAC_Opd* Ternary_Expr_Ast::generate_tac(list<TAC_Stmt*>& statements) {
    // Create result temporary using pre-allocated stemp ID
    // This stemp was allocated in pre_allocate_temps() to ensure proper counter ordering
    TAC_Opd* result;
    if (result_stemp_id >= 0) {
        result = new Temp_TAC_Opd(result_stemp_id, node_data_type, "stemp");
    } else {
        // Fallback if pre_allocate_temps wasn't called - shouldn't happen
        result = TAC_Generator::get_instance()->create_new_stemp(
            node_data_type == FLOAT_DATA_TYPE
        );
    }
...
void Compound_Stmt::pre_allocate_temps() {
    // For non-void functions, allocate shared return stemp FIRST (before any internal statements)
    // This ensures return_stemp gets stemp0, and internal statements (like ternaries) get stemp1+
    ...
    // NOW allocate temps for internal statements (will get stemp1, stemp2, etc...)
    for(auto stmt : statements) {
        stmt->pre_allocate_temps();
    }
    
    // Propagate return labels and stemp to all Return_Stmts
    if(return_label_id >= 0) {
        for(auto stmt : statements) {
            propagate_return_label_to_all_returns(stmt, return_label_id);
        }
    }
...
void FunctionDef_Stmt::pre_allocate_temps() {
    // Allocate return label FIRST

    
    // For non-void functions, allocate return stemp FIRST (before any body statements)
    // This ensures return_stemp gets stemp0, and ternaries in the body get stemp1+
    ...
    // Set return_label_id and return_stemp_id on body Compound_Stmt
    // so it knows they're already allocated and won't allocate again
    if(body && node_data_type != VOID_DATA_TYPE) {
        Compound_Stmt* compound = dynamic_cast<Compound_Stmt*>(body);
        if(compound) {
            compound->set_return_label_id(return_label_id);
            compound->set_return_stemp_id(return_stemp_id);
        }
    }
    
    // Propagate return stemp_id to all Return_Stmts
    if(return_stemp_id >= 0 && body) {
        propagate_return_stemp_id(body);
    }
    
    // NOW pre-allocate body (body will skip allocating return_stemp since we did it, and ternaries will get stemp1+)
    if(body) {
        body->pre_allocate_temps();
    }
}
