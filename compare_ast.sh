...
# Compare generated ast files with valid ast files
...
echo -e "${YELLOW}Running AST comparisons...${NC}\n"
...
for testfile in "$TESTDIR"/*.c; do
    ...
    validfile="$VALIDDIR/$base.correct.ast"
    
    # Per your info, the compiler creates this file automatically
    generatedfile="${testfile}.ast"
    ...
    # 1. Run the compiler with the flag to generate the .ast file
    "$COMPILER" --show-ast "$testfile" > /dev/null 2>&1
    
    # 2. Verify the compiler actually created the file
    if [ ! -f "$generatedfile" ]; then
        echo -e "${RED}$base${NC}: ERROR (Compiler failed to create $base.ast)"
        ((FAILED++))
        continue
    fi
