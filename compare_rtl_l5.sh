...
# Compare generated RTL files with valid RTL files
...
echo -e "${BLUE}==================== RTL COMPARISON (Level-5) ====================${NC}"
...
for testfile in "$TESTDIR"/*.c; do
    ...
    validfile="$VALIDDIR/$base.correct.rtl"
    ...
    # 1. Run the compiler with the flag to generate the .rtl file
    ...
    if diff -Bw "$generatedfile" "$validfile" > /dev/null 2>&1; then
        ...
        diff -Bw "$generatedfile" "$validfile" | head -50
        echo -e "${BLUE}----------------------------------------${NC}"
    fi
done
