#!/usr/bin/env python3
"""
Reorder functions in TAC output using natural/lexicographic sorting.
...
def parse_tac_file(filename):
    """Parse TAC file and extract all functions in order they appear"""
...
def reorder_functions(tac_filename, program_filename):
    """Reorder all functions (including main) using the sorting rule"""
    func_blocks = parse_tac_file(tac_filename)
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: reorder_tac.py <tac_file> <program_file>")
        sys.exit(1)
    
    tac_file = sys.argv[1]
    prog_file = sys.argv[2]
    
    reorder_functions(tac_file, prog_file)
    print(f"Reordered {tac_file}")
