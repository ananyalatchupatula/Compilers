...
void Compute_RTL_Stmt::print(FILE *file) {
    ...
    // For floating-point seq.d, use comma-separated format: dest , opd1 , opd2
    bool seq_float_format = (op == RTL_OP_SEQ && is_float_op);
    
    if (!compare_no_dest && !seq_float_format) {
    dest->print(file);
    fprintf(file, " <- ");
}
    
    if (seq_float_format) {
        // Print: dest , opd1 , opd2
        dest->print(file);
        fprintf(file, " , ");
    }
