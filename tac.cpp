...
string TACGenerator::new_temp() {
    char buf[32];
    sprintf(buf, "temp%d", temp_counter++);
    return string(buf);
}

string TACGenerator::new_stemp() {
    char buf[32];
    sprintf(buf, "stemp%d", stemp_counter++);
    return string(buf);
}

string TACGenerator::new_label() {
    char buf[32];
    sprintf(buf, "Label%d", label_counter++);
    return string(buf);
}
...
void assign_temp_ids(ASTNode *node) {
    ...
    if(strncmp(node->label, "Ternary:", 8) == 0) {
        ...
        // Assign to false part (these get temps after true part)
        if (node->third && strncmp(node->third->label, "False_Part", 10) == 0) {
            assign_temp_ids(node->third->left);
        }
        
        // Do NOT pre-allocate temp for the NOT operation here
        // It will be allocated on-the-fly during code generation, after all branches
        return;
    }
...
string get_temp_name(ASTNode *node) {
    ...
    char buf[32];
    sprintf(buf, "temp%d", node->temp_id);
    return string(buf);
}
...
string get_stemp_name(ASTNode *node) {
    ...
    char buf[32];
    sprintf(buf, "stemp%d", node->stemp_id);
    return string(buf);
}
...
string gen_ternary(ASTNode *node) {
    ...
    // Negate condition for jump to false - always allocate fresh temp
    string cond_neg = tac_gen.new_temp();
